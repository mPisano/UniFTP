...
namespace UniFTP.Server
{
    ...
    /// FTP性能计数器
    /// </summary>
    public class SystemCounter : ICounter
    {
        ...
        public long CurrentNonAnonymousUsers => _currentNonAnonymousUsers.RawValue;

        public long CurrentAnonymousUsers => _currentAnonymousUsers.RawValue;

        public long CurrentConnections => _currentConnections.RawValue;

        public float BytesReceivedPerSec => _bytesReceivedPerSec.NextValue();

        public float BytesSentPerSec => _bytesSentPerSec.NextValue();

        public float BytesTotalPerSec => _bytesTotalPerSec.NextValue();
        ...
        public void Dispose()
        {
            try
            {
                _bytesSentPerSec.Close();
                _bytesReceivedPerSec.Close();
                _bytesTotalPerSec.Close();
                _totalFilesSent.Close();
                _totalFilesReceived.Close();
                _totalFilesTransferred.Close();

                _currentAnonymousUsers.Close();
                _currentNonAnonymousUsers.Close();
                _totalAnonymousUsers.Close();
                _totalNonAnonymousUsers.Close();
                _maximumAnonymousUsers.Close();
                _maximumNonAnonymousUsers.Close();

                _currentConnections.Close();
                _maximumConnections.Close();
                _totalConnectionAttempts.Close();
                _totalLogonAttempts.Close();
                _ftpServiceUptime.Close();

                _commandsExecutedPerSec.Close();
